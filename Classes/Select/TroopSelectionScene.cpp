...
Scene* TroopSelectionScene::createScene() {
...
bool TroopSelectionScene::init() {
    ...
    for (auto type : types) {
        ...
        else if (type == TroopType::BOMBER) color = Color3B::BLUE;
        else if (type == TroopType::GIANT) color = Color3B::YELLOW;

        auto colorBlock = LayerColor::create(Color4B(color), 80, 80);
        colorBlock->setPosition(Vec2(xPos - 40, centerY)); // Center anchor simulation
        this->addChild(colorBlock, 1);

        // Invisible button on top of color block to handle click (Add)
        auto touchBtn = ui::Button::create();
        touchBtn->ignoreContentAdaptWithSize(false);
        touchBtn->setContentSize(Size(80, 80));
        touchBtn->setPosition(Vec2(xPos, centerY + 40)); 
        touchBtn->addClickEventListener([this, type](Ref* sender) {
            this->onTroopSelected(type);
        });
        this->addChild(touchBtn, 2);
        ...
        // C. Cost/Space Info
        auto spaceLabel = Label::createWithSystemFont("Space: " + std::to_string(info.housingSpace), "Arial", 14);
...
void TroopSelectionScene::updateLabels() {
    ...
    capacityLabel->setString(capText);

    // Update Counts
    for (auto& pair : countLabels) {
