/*************************************************************
* @file     : SceneMap.cpp
* @function ：主地图场景核心实现 - 家乡村庄管理+建筑交互+资源系统
* @author   : 叶芷含 齐颖 俞筱雨
* @note     : 1.单例模式实现主地图场景，封装地图初始化、UI创建、事件监听核心逻辑；
*             2.管理家乡村庄地图实例，支持建筑放置/移动/升级/移除全流程交互；
*             3.实现金币/圣水资源系统，包含产速计算、容量限制、UI实时刷新功能；
*             4.适配多平台触摸/鼠标事件，支持建筑右键菜单、拖拽预览、位置校验；
*             5.集成战斗/商店/选兵功能按钮，支持场景切换与战斗关卡选择；
*             6.自动初始化默认建筑布局（大本营/金矿/圣水设施/防御建筑等）；
*             7.封装事件监听器重初始化逻辑，保证场景切换后交互正常；
*             8.支持建筑升级后产速/容量实时更新，适配资源系统动态调整
**************************************************************/
