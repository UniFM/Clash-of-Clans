/*************************************************************
@file : BuildingPreview.cpp
@function ：建筑预览与移动核心实现 - 多模式建筑拖拽 / 放置 / 位置校验
@author : 叶芷含
@note : 1. 支持建筑预览放置、普通移动、简单拖拽三种模式；
        2. 含网格位置校验、占位标记、透明度调整逻辑；
        3. 提供确认 / 取消按钮交互，按钮跟随建筑位置更新；
        4. 拖拽阈值判定预览拖拽启动，超出范围自动回退原位置；
        5. 支持建筑放置 / 移动事件分发，适配多类型建筑加载
**************************************************************/
...
void BuildingPreview::startPreview(BuildingType buildingType)
{
    ...
    switch (buildingType){ 
        case BuildingType::GOLD_MINE:
            building = GoldMine::create(1);
            break;
        case BuildingType::ELIXIR_COLLECTOR:
            building = ElixirCollector::create( 1);
            break;
        case BuildingType::BARRACKS:
            building = Barracks::create(1);
            break;
        case BuildingType::ARCHER_TOWER:
            building = ArcherTower::create(1);
            break;
        case BuildingType::CANNON:
            building = Cannon::create( 1);
            break;
        case BuildingType::GOLD_STORAGE:
            building = GoldStorage::create(1);
            break;
        case BuildingType::ELIXIR_STORAGE:
            building = ElixirStorage::create(1);
            break;
        default:
